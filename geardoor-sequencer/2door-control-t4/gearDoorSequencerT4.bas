;
; RC Gear door seaquencer-T4 for PICAXE 08M2
;
...
; Pins
symbol LED_PIN = 0 ; LED on/offout, ON when gear down complete like the landing light.
symbol DOOR_PIN = 1 ; gear door(MAIN/NOSE) pulse out
symbol GEAR_PIN = 2 ; gear pulse out
symbol SW_PIN = 3 ; Gear switch pulse input 
symbol DOOR2_PIN = 4 ; gear door(REAR) pulse out
;
; Vaiables & Constant
;
; reverse
symbol GEAR_REVERSE = 1 ; 1: reverse 0;non
symbol DOOR_REVERSE = 0 ; 1: reverse main door 0;non
symbol DOOR2_REVERSE = 0 ; 1: reverse rear door 0;non
...
; Current door(MAIN/NOSE) pulse
symbol pulseDoor = w4
	symbol DOOR_MAX_POS =  1550; 2.1ms
	symbol DOOR_MIN_POS =  650; 0.8ms
...
symbol pulseDoor2 = w5
	...
	symbol DOOR2_PLUSE_INCREMENT = 8 ; incriment next pulse out for door(REAR) open/close speed, To Fist, increase the value.


; open/close wait count from gear down/up out to start for door(REAR)
 symbol countWaitCloseDoor = w6
	symbol DOOR2_OPEN_WAIT = 100 ; msec(>=20) wait for from output gear DOWN to start of door(REAR) open
...
outPulseGearDoors:
	...
	if pulseDoor > 0 then
		pulsout DOOR_PIN, pulseDoor ; door (MANI/NOSE)
	endif
	if pulseDoor2 > 0 then
		pulsout DOOR2_PIN, pulseDoor2 ; door(REAR) 
	endif
	return
